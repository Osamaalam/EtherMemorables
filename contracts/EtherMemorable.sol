...
contract EtherMemorable is ReentrancyGuard {
    ...
    /* number of items minting,
     ...
     constructor() {
         //set the owner
         //msg.sender who deploys the contract
    ...
    function OwnerTokenId(address nftContract,uint256 id) public view returns(address){
      return IERC721(nftContract).ownerOf(id);
    }
    ...
    function TransferItem(
        ...
        uint tokenNo = idToMarketToken[itemNo].tokenId;
        address ownerToken =   IERC721(nftContract).ownerOf(tokenNo);
        require(msg.sender == ownerToken, "ERC721: Only token owner can tranfer");
        IERC721(nftContract).setApprovalForAll(msg.sender, true);
        IERC721(nftContract).transferFrom(msg.sender, to, tokenNo);
        idToMarketToken[itemNo].owner = payable(to);
        IERC721(nftContract).setApprovalForAll(msg.sender, false);

        }
        function massageSender() public view returns(address){
          return msg.sender;
        }
